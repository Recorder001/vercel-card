...
function clamp(val: string | null, def = 0): number {
  const n = parseFloat(val ?? String(def));
...
function GradeScale({ color, marks, pt }: { color: string; marks: {at:number; label:string}[]; pt: number }) {
  const pct = Math.min(100, pt);
  return (
    <div style={{ position: 'relative', marginTop: 20, marginBottom: 28, display: 'flex', flexDirection: 'column' }}>
      <div style={{ position: 'absolute', left: `${pct}%`, top: -18, transform: 'translateX(-50%)', color: C.red, fontFamily: DSP, fontSize: 11, whiteSpace: 'nowrap' }}>▼YOU</div>
      ...
      {marks.map((m, i) => {
        ...
        return (
          <div key={i} style={{ position: 'absolute', left: `${m.at}%`, top: 18, transform: tform, fontFamily: BDY, fontSize: 10, fontWeight: 800, color: active ? color : C.muted, whiteSpace: 'nowrap' }}>
            {m.label}
          </div>
        );
      })}
